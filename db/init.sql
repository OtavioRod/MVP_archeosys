...
CREATE TABLE notas (
    ...
    id_alunos INT REFERENCES alunos(id_alunos),
    id_disciplinas INT REFERENCES disciplinas(id_disciplinas),
    bimestre INT NOT NULL CHECK (
        bimestre BETWEEN 1 AND 4
    ),
    nota DECIMAL(5, 2) CHECK (
...
CREATE TABLE solicitacoes_correcao (
    ...
    --referencia ao ID da nota ou presenca
...
CREATE TABLE log_alteracoes (
    ...
    --obs: Nome da tabela Afetada
    id_registro INT NOT NULL,
    -- obs: ID do registro alterado
    id_usuarios INT NOT NULL REFERENCES usuarios(id_usuarios),
    --obs: Quem fez a alteracao
    acao VARCHAR(10) NOT NULL,
    -- obs:(INSERT, UPDATE, DELETE)
    data_alteracao TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    -- obs: data/hora da alteracao
...
CREATE OR REPLACE FUNCTION verificar_presenca_unica() RETURNS TRIGGER AS $$ BEGIN IF EXISTS (
        SELECT 1
        FROM presencas
        WHERE id_alunos = NEW.id_alunos
            AND id_disciplinas = NEW.id_disciplinas
            AND data = NEW.data
    ) THEN RAISE EXCEPTION 'Presença já registrada para este aluno nesta data e disciplina';
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
-- TRIGGER: aplicar verificação de presença única
CREATE TRIGGER trg_presenca_unica BEFORE
INSERT ON presencas FOR EACH ROW EXECUTE FUNCTION verificar_presenca_unica();
-- FUNCTION: log para alterações em notas
CREATE OR REPLACE FUNCTION log_update_nota() RETURNS TRIGGER AS $$ BEGIN
...
AFTER
UPDATE ON notas FOR EACH ROW EXECUTE FUNCTION log_update_nota();